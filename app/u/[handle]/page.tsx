// /workspaces/Vext/app/u/[handle]/page.tsx
'use client';
...
export default function PersonalHandlePage() {
  ...
  const displayName = useMemo(() => {
    ...
    return (
      profile.fullName ||
      `@${profile.personalUsername || profile.username || 'unknown'}`
    );
  }, [profile]);

  // For personal profile, prefer personal avatar, then fall back to business avatar
  const avatarUrl =
    profile?.profilePhoto || profile?.businessProfilePhoto || '';
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // 1) Try mapping: /usernames/{handle}
        const mapRef = doc(db, 'usernames', handle);
        const mapSnap = await getDoc(mapRef);
        if (mapSnap.exists()) {
          uid = (mapSnap.data() as any).uid || null;
        }

        // 2) Fallback: search users by personalUsername or username
        if (!uid) {
          let userSnap = await getDocs(
            query(
              collection(db, 'users'),
              where('personalUsername', '==', handle),
              limit(1)
            )
          );
          ...
          // If still empty and rawHandle differs (casing), try rawHandle
          if (userSnap.empty && rawHandle && rawHandle !== handle) {
            let rawSnap = await getDocs(
              query(
                collection(db, 'users'),
                where('personalUsername', '==', rawHandle),
                limit(1)
              )
            );
            if (rawSnap.empty) {
              rawSnap = await getDocs(
                query(
                  collection(db, 'users'),
                  where('username', '==', rawHandle),
                  limit(1)
                )
              );
            }
          ...
          if (!userSnap.empty) {
            ...
            // Backfill mapping so next lookup is instant
            try {
              await setDoc(mapRef, { uid });
            } catch (e) {
              console.warn('backfill usernames failed:', e);
            }
          }
        }
        ...
        console.error('personal handle load error', err);
        setNotFound(true);
        setLoading(false);
      }
    })();
  ...
  const toggleFollow = async () => {
    if (!user) return alert('Sign in to follow users');
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-white">
        <h1 className="text-xl font-semibold text-gray-800 mb-2">
          User {displayHandle} not found.
        </h1>
        <p className="text-gray-500 mb-4">
          The personal profile for this handle could not be located.
        </p>
        <button
          onClick={() => router.push('/')}
  ...
  // ---------- main UI (similar to /creator/[userId], but for personal profile) ----------
  return (
      ...
      <div className="px-4 pt-8 sm:pt-12 sm:px-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:gap-10">
          ...
          <div className="mt-4 sm:mt-0 flex-1">
            ...
            {/* Optional business line if they’re also a provider */}
